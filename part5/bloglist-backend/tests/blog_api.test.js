...
test('deleting a blog without token fails with status 401', async () => {
  const blogsAtStart = await helper.blogsInDb()
  const blogToDelete = blogsAtStart[0]
...
test('deleting a blog with another user token fails with status 403', async () => {
  const blogsAtStart = await helper.blogsInDb()
  const blogToDelete = blogsAtStart[0]

  const otherPasswordHash = await bcrypt.hash('password2', 10)
